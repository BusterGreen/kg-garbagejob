...
KGCore.Functions.CreateCallback('kg-garbagejob:server:NewShift', function(source, cb, continue)
    ...
    if CanPay(Player) or continue then
        ...
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.not_enough', { value = Config.TruckPrice }), 'error')
    end
    cb(shouldContinue, nextStop, bagNum, totalNumberOfStops)
end)

RegisterNetEvent('kg-garbagejob:server:payDeposit', function()
    local Player = KGCore.Functions.GetPlayer(source)
...
KGCore.Functions.CreateCallback('kg-garbagejob:server:NextStop', function(source, cb, currentStop, currentStopNum, currLocation)
    local Player = KGCore.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    ...
    if (math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        exports['kg-inventory']:AddItem(source, 'cryptostick', 1, false, false, 'kg-garbagejob:server:NextStop')
        TriggerClientEvent('kg-inventory:client:ItemBox', source, KGCore.Shared.Items['cryptostick'], 'add')
        TriggerClientEvent('KGCore:Notify', source, Lang:t('info.found_crypto'))
    end
    ...
    if distance <= 20 then
        ...
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.too_far'), 'error')
    end
    cb(shouldContinue, newStop, newBagAmount)
end)

KGCore.Functions.CreateCallback('kg-garbagejob:server:EndShift', function(source, cb)
    local Player = KGCore.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    local status = false
...
RegisterNetEvent('kg-garbagejob:server:PayShift', function(continue)
    local src = source
    ...
    if Routes[CitizenId] ~= nil then
        ...
        if tonumber(Routes[CitizenId].stopsCompleted) < tonumber(Routes[CitizenId].totalNumberOfStops) then
            depositPay = 0
            TriggerClientEvent('KGCore:Notify', src, Lang:t('error.early_finish', { completed = Routes[CitizenId].stopsCompleted, total = Routes[CitizenId].totalNumberOfStops }), 'error')
        end
        if continue then
            depositPay = 0
        end
        ...
        TriggerClientEvent('KGCore:Notify', src, Lang:t('success.pay_slip', { total = totalToPay, deposit = payoutDeposit }), 'success')
        Routes[CitizenId] = nil
    else
        TriggerClientEvent('KGCore:Notify', source, Lang:t('error.never_clocked_on'), 'error')
    end
end)

KGCore.Commands.Add('cleargarbroutes', 'Removes garbo routes for user (admin only)', { { name = 'id', help = 'Player ID (may be empty)' } }, false, function(source, args)
    ...
    TriggerClientEvent('KGCore:Notify', source, Lang:t('success.clear_routes', { value = count }), 'success')
    Routes[CitizenId] = nil
end, 'admin')
